...
class TaskApp(App):
    ...
    def safe_switch_to(self, name):
        try:
            ...
            if name not in names:
                # attempt to (re)create summary or notepad if missing
                if name == "summary":
                    # rebuild and add summary screen safely
                    self._recreate_summary_screen()
                elif name == "notepad":
                    # notepad should exist; if missing, create a simple one
    ...
    def _recreate_summary_screen(self):
        """
        If summary screen was removed for any reason, recreate it and add to sm.
        """
        try:
            ...
            with summary_root.canvas.before:
                Color(0.02, 0.03, 0.05, 1)
                rect = Rectangle(pos=summary_root.pos, size=summary_root.size)
            # header + container (simpler)
            ...
            summary_screen.add_widget(summary_root)
            self.sm.add_widget(summary_screen)
            # point the instance to the rebuilt container for later use (best-effort)
    ...
    def load_tasks_for_date(self):
        try:
            ...
            # ensure notepad_text exists (it should)
            try:
    ...
    # ---------------- Summary screen building (readable/organized) ----------------
    ...
    def open_notepad_screen(self):
        # ensure notepad exists then switch
        self.safe_switch_to("notepad")
        # notepad content already loaded by load_tasks_for_date (on nav between days)
        # but ensure text is current (in case user switched days through other means)
        try:
            date_str = self.current_date.isoformat()
            data = safe_load_data()
            note = ""
